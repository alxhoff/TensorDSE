...
profile:
	...
	#git fetch https://git@github.com/alxhoff/TensorDSE.git
	#git reset --hard origin/$(BRANCH)
	${MAKE} -C docker profile USBMON=$(USBMON) MODEL=$(MODEL) COUNT=$(COUNT) MODEL_SUMMARY=$(MODEL_SUMMARY) ARCHITECTURE_SUMMARY=$(ARCHITECTURE_SUMMARY) PROFILING_COSTS=$(PROFILING_COSTS) PLATFORM=$(PLATFORM) MULTI_MODEL=$(MULTI_MODEL) MODEL_NAME=$(MODEL_NAME)
