...
def ProfileModelLayers(
    ...
    if not isCPUavailable():
        ...
        print(f"[PROFILE MODEL LAYERS] CPU is available on this machine!")
        for layer in model_summary["models"][0]["layers"]:
            m = ProfileLayer(Model(layer, "cpu", parent_model), count, "CPU", platform)
            models["cpu"].append(m)
    ...
    if platform == "desktop":
        if not isTPUavailable():
            ...
            print(f"[PROFILE MODEL LAYERS] TPU is available on this machine!")
    ...
    print("[PROFILE MODEL LAYERS] Finished")
