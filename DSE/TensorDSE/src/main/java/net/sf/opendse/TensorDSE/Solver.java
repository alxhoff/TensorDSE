    ...
    private Double getCommunicationCost(Task comm, Resource target_resource) {
        ...
        if (comm != null) {
            ...
            for (Resource res : routed_targets)
                if (res.getId().equals("pci") || res.getId().equals("usb")) {
                    Pair<Double, Double> comms = operation_costs.GetCommCost(
                            target_resource.getId().replaceAll("\\d", ""),
                            comm.getAttribute("type"), comm.getAttribute("dtype"));
                    comm_cost += comms.getValue0() + comms.getValue1();
                }
        }
    ...
    public Boolean calculateTaskFinishTimes() {
        ...
        try {
            ...
            for (Task starting_task : this.starting_tasks) {
                ...
                Resource tr = getTargetResource(starting_task);
                Double cc = getCommunicationCost(dependent_comm, tr);
                Double ec = getExecutionCost(starting_task, tr);
                ILPTask ilp_task = ilps.initILPTask(starting_task.getId(), tr, cc, ec, model);
                ...
                while (count > 0) {
                    ...
                    tr = getTargetResource(starting_task);
                    cc = getCommunicationCost(dependent_comm, tr);
                    ec = getExecutionCost(starting_task, tr);
                    ilp_task = ilps.initILPTask(starting_task.getId(), tr, cc, ec, model);
                    ilp_task_branch.add(ilp_task);
