...
cd ${TENSORFLOW_DIR}
bazel clean
bazel build -s -c opt --copt="-DMESA_EGL_NO_X11_HEADERS" tensorflow/lite/delegates/gpu:libtensorflowlite_gpu_delegate.so
cp bazel-bin/tensorflow/lite/delegates/gpu/libtensorflowlite_gpu_delegate.so ${TENSORFLOW_ENV_DIR}/bazel-output/

# --------------------------------------------------------------------------------------------------------------------------------------------------
#   In case of Building Tensorflow Lite GPU Delegate Dynamic Library (Open CL support) for aarch64
